...
![My Image](resume-site/images/resume-design.png)
...
Then set-up [SAM CLI](https://docs.aws.amazon.com/serverless-application-model/latest/developerguide/serverless-sam-cli-command-reference.html)
...
And I deployed ressources with SAM:
...
I bought a domain name, and then added an A Record which will be used to access the website:
```yml
  MyRoute53Record:
...
```
*
To serve secure content I needed a HTTPS certificate so I used ACM to request one, with DNS validation :
```yml
  MyCertificate:
...
*
Then I attached that certificate to my Cloudfront distribution:
```yml
  # MyDistribution:
    # Properties:
      # DistributionConfig:
...
At that point, the deployment got stuck on validating the certificate. The only solution I found was to
validate it manually, so I went to ACM and pressed `Create a record in Route53`to unlock this issue.
It worked and a record was created in Route53, and deployment successful!
*
However, I still couldn't access my distribution through my custom domain,
so I needed to connect the DNS record and the distribution, by adding this line to my distribution setup:
...
After figuring it out, it looked like this :
```yaml
  VisitCountFunction:
